...
//!
//! Ported from hdrprobe's `bdiso::mpls`; adapted to `String` errors.
...
// Bounds-safe big-endian readers: OOB reads 0; the item loop is bounded by the
// buffer and a count cap, so 0 can't run away.
...
pub fn parse(data: &[u8]) -> Result<Playlist> {
    if data.len() < 16 || &data[0..4] != b"MPLS" {
        return Err("not an MPLS playlist".into());
    }
    let version = &data[4..8];
    if !matches!(version, b"0100" | b"0200" | b"0300") {
        return Err(format!("unsupported MPLS version {}", String::from_utf8_lossy(version)));
    }
    ...
    if pl_start < 16 || pl_start + 10 > data.len() {
        return Err("PlayList block offset out of range".into());
    }
...
pub(crate) mod tests {
    ...
    /// segments. Also used by the sibling module's tests to stuff playlist
